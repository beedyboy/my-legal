...
const AddStock = ({mode, open, handleClose, initial_data}) => {
  ...
  const [title, setTitle]  = useState('Add Stock');
    const [formState, setFormState] = useState({ 
     values: {  id: '', stock_name: '',  quantity: '', expiry: '',  price: '', product_id: ''},
      touched: {},
        errors: {}
      }); 
  useEffect(() => {
    ...
    return () => {
      setFormState(prev => ({
        ...prev,
      values: { 
        ...prev.values,  id: '', stock_name: '',  quantity: '', expiry: '',  price: '', product_id: ''}
      }))
    }
  }, [initial_data, mode]);
  useEffect(() => {
          ...
          <Row>
              <Col md="12"> 
                <FormGroup>
                  ...
                  invalid={hasError('stock_name') || exist}
                  />
                   <FormFeedback>  
