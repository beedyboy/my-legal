...
const StaffForm = ({mode, open, handleClose, initial_data}) => {
  ...
  const [title, setTitle]  = useState('Add Staff');
    const [formState, setFormState] = useState({ 
     values: {  id: '', firstname: '', lastname: '', password: '',  email: '',  phone: '',  address: ''},
      touched: {},
        errors: {}
      });
  useEffect(() => {
    ...
    return () => {
      setFormState(prev => ({
        ...prev,
      values: { 
        ...prev.values, id: '', firstname: '', lastname: '', password: '',  email: '', phone: '',  address: ''}
      }))
    }
  }, [initial_data, mode]);
  useEffect(() => {
    ...
    setFormState(formState => ({
      ...formState,
      isValid: errors.firstname.error || errors.lastname.error || errors.password.error || errors.email.error ?  false: true,
      errors: errors || {}
    }));
  }, [formState.values]);
...
const handleSubmit = e => {
  ...
  const resetForm = () => {
    setFormState(prev => ({
      ...prev,
      values: { ...prev.values,  id: '', firstname: '', lastname: '', password: '',   email: '', phone: '',  address: ''},
      touched: {
        ...formState.touched,
        firstname: false,
        lastname: false,
